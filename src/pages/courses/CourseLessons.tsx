                ...
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }}>
                  <Card sx={{ 
                    ...
                    <CardActionArea component={RouterLink} to={`/courses/${courseId}/${lesson.id}/topic`} sx={{ p: 2, height: '100%' }}>
                      ...
                      <Typography sx={{ fontWeight: 800, fontSize: '0.9rem', mt: 0.5, lineHeight: 1.2 }}>{getText(lesson.title)}</Typography>
                    </CardActionArea>
                  </Card>
                </motion.div>
              </Grid>
                  ...
                  <Card sx={{ 
                    ...
                    <CardActionArea component={RouterLink} to={`/courses/${courseId}/${lesson.id}`} sx={{ p: 2, height: '100%' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Typography sx={{ fontSize: '0.65rem', fontWeight: 900, color: 'text.disabled' }}>{t('lesson.laboratory').substring(0, 3).toUpperCase()} {index + 1}</Typography>
