...
import { 
  PlayCircle, ChevronRight, CheckCircle2, Loader2, Lock, Rocket, 
...
import { 
  ...
  LinearProgress, Stack, Alert, Tabs, Tab, IconButton, Tooltip 
} from '@mui/material';
  ...
  return (
    <motion.div style={{ position: 'relative' }} whileHover={{ y: -5 }} animate={error ? { x: [-10, 10, -10, 10, 0] } : {}} transition={{ duration: 0.4 }}>
      <Box sx={{ position: 'absolute', top: 15, right: 15, zIndex: 10 }}>
        {!isDeleting ? (
          <IconButton onClick={(e) => { e.stopPropagation(); setIsDeleting(true); }} size="small" sx={{ color: 'rgba(255,255,255,0.2)', '&:hover': { color: '#ef4444' } }}>
            <Trash2 size={18} />
          </IconButton>
        ) : (
          <Stack direction="row" spacing={1} sx={{ bgcolor: '#1a1a1a', p: 0.5, borderRadius: '8px', border: '1px solid #ef4444', alignItems: 'center' }}>
            <TextField size="small" placeholder="PIN" type="password" value={deletePin} autoFocus onChange={(e) => setDeletePin(e.target.value)} slotProps={{ htmlInput: { maxLength: 4, style: { color: 'white', fontSize: '12px', width: '40px', padding: '4px', textAlign: 'center' } } }} />
            <IconButton onClick={handleConfirmDelete} size="small" sx={{ color: '#4ade80' }}><Check size={16} /></IconButton>
            <IconButton onClick={(e) => { e.stopPropagation(); setIsDeleting(false); }} size="small" sx={{ color: 'gray' }}><X size={16} /></IconButton>
          </Stack>
        )}
      </Box>

      <Card sx={{ p: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', borderRadius: '2.5rem', border: '2px solid', borderColor: error ? 'error.main' : 'rgba(255,255,255,0.1)', bgcolor: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', height: '100%', transition: 'all 0.3s ease', '&:hover': { borderColor: '#a855f7' } }}>
        <Avatar sx={{ width: 80, height: 80, mb: 2, bgcolor: '#a855f7', fontSize: '2.2rem', fontWeight: 900, border: '3px solid rgba(255,255,255,0.1)' }}>{student.name.charAt(0)}</Avatar>
        <Typography variant="h5" sx={{ fontWeight: 900, mb: 3 }}>{student.name}</Typography>
        <Stack spacing={2} sx={{ width: '100%' }}>
          <TextField fullWidth type="password" placeholder="PIN" value={pin} onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && onLogin(student, pin)} autoComplete="off" slotProps={{ htmlInput: { maxLength: 4, style: { textAlign: 'center', letterSpacing: '0.5em', fontWeight: 'bold', color: 'white' } } }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '1.2rem', bgcolor: 'rgba(0,0,0,0.2)' } }} />
          <Button variant="contained" fullWidth size="large" onClick={() => onLogin(student, pin)} startIcon={<Lock size={18} />} sx={{ borderRadius: '1.2rem', py: 1.5, fontWeight: 900, bgcolor: '#a855f7', '&:hover': { bgcolor: '#9333ea' } }}>{t('auth.login')}</Button>
        </Stack>
      </Card>
    </motion.div>
  );
...
export default function StudentDashboard() {
  ...
  if (loading) return <Box sx={{ minHeight: '100vh', display: 'flex', bgcolor: '#0f0f0f', alignItems: 'center', justifyContent: 'center' }}><Loader2 className="animate-spin" color="#a855f7" size={48} /></Box>;

  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#0a0a0a', color: 'white', pb: 8 }}>
      {/* HEADER CORREGIT */}
      <Box sx={{ height: '80px', bgcolor: 'rgba(18, 18, 18, 0.8)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, px: { xs: 2, md: 8 }, zIndex: 1100 }}>
        <Stack direction="row" spacing={1} onClick={() => navigate('/')} sx={{ alignItems: 'center', cursor: 'pointer' }}>
          <Rocket size={28} color="#fff" fill="#a855f7" style={{ transform: 'rotate(-45deg)' }} />
          <Typography variant="h5" sx={{ fontWeight: 900 }}>MOOC <Box component="span" sx={{ color: '#a855f7' }}>2026</Box></Typography>
        </Stack>
        ...
        <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
          ...
          <Stack direction="row" sx={{ bgcolor: 'rgba(255,255,255,0.05)', borderRadius: '12px', p: 0.5 }}>
            {['ca', 'es', 'en'].map((lng) => (
              <Button 
                ...
                sx={{ 
                  minWidth: '40px', px: 1, fontSize: '0.75rem', fontWeight: 800,
          ...
          {/* BOTÓ CORREGIT: SI NO HI HA USER, VA A '/' */}
          <Button 
            onClick={() => {
              if (selectedStudent) {
                handleLogoutAction();
            ...
            variant="contained" 
            sx={{ borderRadius: '12px', px: 4, fontWeight: 800, background: 'linear-gradient(90deg, #a855f7 0%, #6366f1 100%)' }}
          >
            {selectedStudent ? t('auth.logout') : t('auth.start')}
          </Button>
        </Stack>
      </Box>
          ...
          {!selectedStudent ? (
            <motion.div key="login" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Box sx={{ display: 'grid', gap: 5, gridTemplateColumns: { xs: '1fr', lg: '350px 1fr' } }}>
                <Card sx={{ p: 4, borderRadius: '2.5rem', bgcolor: 'rgba(255,255,255,0.02)', border: '2px dashed rgba(168,85,247,0.3)', height: 'fit-content' }}>
                  <Stack spacing={3} component="form" onSubmit={handleCreateStudent}>
                    <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
                      <UserPlus color="#a855f7" />
                      <Typography variant="h6" sx={{ fontWeight: 900 }}>{t('dashboard.create_user_title')}</Typography>
                    </Stack>
                    <TextField fullWidth label={t('dashboard.name_label')} variant="filled" value={newName} onChange={e => setNewName(e.target.value)} sx={{ bgcolor: 'rgba(0,0,0,0.3)', borderRadius: '12px' }} />
                    <TextField fullWidth label={t('dashboard.email_label')} variant="filled" value={newEmail} onChange={e => setNewEmail(e.target.value)} sx={{ bgcolor: 'rgba(0,0,0,0.3)', borderRadius: '12px' }} />
                    <TextField fullWidth label={t('dashboard.pin_label')} variant="filled" type="password" value={newPin} onChange={e => setNewPin(e.target.value)} slotProps={{ htmlInput: { maxLength: 4 } }} sx={{ bgcolor: 'rgba(0,0,0,0.3)', borderRadius: '12px' }} />
                    <Button type="submit" variant="outlined" fullWidth sx={{ borderRadius: '12px', py: 1.5, borderColor: '#a855f7', color: '#a855f7', fontWeight: 800 }}>{t('dashboard.register_btn')}</Button>
                  </Stack>
                </Card>
            ...
            <motion.div key="dash" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ display: 'grid', gap: 5, gridTemplateColumns: { xs: '1fr', md: '320px 1fr' } }}>
                <Stack spacing={3}>
                  <Card sx={{ p: 4, borderRadius: '2.5rem', textAlign: 'center', bgcolor: 'rgba(255,255,255,0.05)', position: 'relative', border: '1px solid rgba(168, 85, 247, 0.2)' }}>
                    {actionLoading && <LinearProgress sx={{ position: 'absolute', top: 0, left: 0, right: 0 }} color="secondary" />}
                    <Avatar sx={{ width: 80, height: 80, mx: 'auto', mb: 2, bgcolor: '#a855f7', fontWeight: 900, fontSize: '2rem' }}>{selectedStudent.name.charAt(0)}</Avatar>
                    <Typography variant="h6" sx={{ fontWeight: 900 }}>{selectedStudent.name}</Typography>
                    <Box sx={{ mt: 3, p: 2, bgcolor: 'rgba(0,0,0,0.2)', borderRadius: '1.2rem' }}>
                      <Typography variant="caption" sx={{ opacity: 0.5, fontWeight: 800 }}>{t('dashboard.average')}</Typography>
                      <Typography variant="h4" sx={{ fontWeight: 900, color: '#a855f7' }}>
                ...
                <Box>
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2, mb: 6 }}>
                    {allCourses.map(course => (
                      <Box key={course.id} sx={{ position: 'relative' }}>
                        <Card onClick={() => setExpandedCourse(expandedCourse === course.id ? null : course.id)} sx={{ p: 2.5, cursor: 'pointer', borderRadius: '1.5rem', display: 'flex', alignItems: 'center', gap: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '2px solid', borderColor: expandedCourse === course.id ? '#a855f7' : 'transparent', transition: '0.2s' }}>
                          <Box sx={{ p: 1.5, bgcolor: 'rgba(168, 85, 247, 0.1)', borderRadius: '12px' }}><CourseIcon title={course.title} /></Box>
                          <Box sx={{ flex: 1 }}>
                          ...
                          <Stack direction="row" spacing={0.5} sx={{ alignItems: "center" }}>
                            <Tooltip title={t('dashboard.reset_course_tooltip')}>
                              <IconButton onClick={(e) => handleResetCourse(e, course.id)} size="small" sx={{ color: 'rgba(255,255,255,0.2)', '&:hover': { color: '#ef4444' } }}><RotateCcw size={16} /></IconButton>
                            </Tooltip>
                        ...
                        <AnimatePresence>
                          {expandedCourse === course.id && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, marginTop: '8px' }}>
                              <Box sx={{ p: 1.5, bgcolor: '#1a1a1a', borderRadius: '1.2rem', border: '1px solid rgba(168, 85, 247, 0.3)', maxHeight: '200px', overflowY: 'auto' }}>
                                {course.content?.map(lesson => (
                                  <Box key={lesson.id} onClick={() => navigate(`/courses/${course.id}/${lesson.id}`)} sx={{ p: 1.2, borderRadius: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', '&:hover': { bgcolor: 'rgba(255,255,255,0.08)' } }}>
                                    <Typography variant="caption">{lesson.title}</Typography>
                                    {dbProgress[`${course.id}_${lesson.id}`] ? <CheckCircle2 size={18} color="#4ade80" /> : <PlayCircle size={18} color="#a855f7" />}
                                  </Box>
                                ))}
                              </Box>
                            </motion.div>
                  ...
                  <Card sx={{ borderRadius: '2rem', bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                    <Tabs value={rankingTab} onChange={(_, val) => setRankingTab(val)} variant="scrollable" sx={{ bgcolor: 'rgba(0,0,0,0.2)', '& .MuiTabs-indicator': { bgcolor: '#a855f7' } }}>
                        ...
                        {rankedStudentsByCourse.map((s, idx) => {
                          ...
                          return (
                            <Box key={s.id} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              <Typography sx={{ width: 25, fontWeight: 900, color: idx < 3 ? '#ffb700' : 'rgba(255,255,255,0.2)' }}>{idx + 1}</Typography>
                              <Avatar sx={{ width: 32, height: 32, bgcolor: isMe ? '#a855f7' : '#333' }}>{s.name.charAt(0)}</Avatar>
                              <Typography sx={{ flex: 1, fontWeight: isMe ? 900 : 400, color: isMe ? '#a855f7' : 'white' }}>
                                {s.name} {isMe && `(${t('dashboard.you')})`}
                              </Typography>
                              <Typography sx={{ fontWeight: 900, color: '#a855f7' }}>{getCourseProgress(allCourses[rankingTab], s.id)}%</Typography>
                            </Box>
                          );
                        })}
                      </Stack>
